...
func Run(s *options.SchedulerServer) error {
	...
	// ... 将当前scheduler的配置信息设置成全局的map(configs = map[string]*Config{})
	// ... 并返回map保存的value的空结构体(Config{})
	if c, err := configz.New("componentconfig"); err == nil {

		// ...将当前scheduler的配置写入到上一步返回的空结构体(Config{})
	...
	// ... scheduler每秒处理的最大请求并发数(QPS)
	// ... scheduler限制的最大请求数(burst)
	...
	// ... pprof是性能分析工具
	// ... 启动代码检查功能，包括指标使用情况，心跳检查
	// ... 该服务会监听在s.Address:s.Port
	go func() {
	...
	*/
	...
	// ... 创建event，由于记录K8安装到运行整个过程中发生的重要状态变更
	eventBroadcaster := record.NewBroadcaster()
	// ... 创建event记录器，用于记录scheduler产生的events
	// ... 被记录的事件会被发送到上一步创建的eventBroadcaster
	config.Recorder = eventBroadcaster.NewRecorder(api.EventSource{Component: s.SchedulerName})

	// ... 日志记录scheduler产生的events
...
/*
// ... 分成两种调度策略
// ... 用户指定配置文件(即根据用户指定的配置创建过滤策略和打分策略)
// ... 用户没有指定配置，采用默认配置(即scheduler本身提供的规则创建过滤策略和打分策略)
*/
