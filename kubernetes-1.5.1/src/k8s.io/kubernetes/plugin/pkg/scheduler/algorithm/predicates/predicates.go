...
func isVolumeConflict(volume api.Volume, pod *api.Pod) bool {
	...
	for _, existingVolume := range pod.Spec.Volumes {
		...
		// Amazon EBS不允许不同的Pod挂载同一个卷。
		// Amazon Elastic Block Store(EBS)_持久性数据块级存储卷
...
//检查给定的zone限制前提下，检查如果在此主机上部署Pod是否存在卷冲突。假定一些volumes可能有zone调度约束，
// VolumeZonePredicate根据volumes自身需求来评估pod是否满足条件。
// 必要条件就是任何volumes的zone-labels必须与节点上的zone-labels完全匹配。
// 节点上可以有多个zone-labels的约束（比如一个假设的复制卷可能会允许进行区域范围内的访问）。
// 目前，这个只对PersistentVolumeClaims支持，而且只在PersistentVolume的范围内查找标签
...
//检查主机的资源是否满足Pod的需求。根据实际已经分配的资源量做调度，而不是已使用的资源量做调度
...
//... 在pod配置文件的spc部分可以配置nodeSelector来约束该pod必须在指定的node上调度
/*
apiVersion: v1
kind: PodpodMatchesNodeLabels
  labels:
    env: test
spec:
  containers:
...
//podAffinity主要解决POD可以和哪些POD部署在同一个拓扑域中的问题
